...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Provider.DesignTime", "Provider")], addDefaultProbingLocation=true)

    let ns = "Provider.Provided"
    let asm = Assembly.GetExecutingAssembly()

    // check we contain a copy of runtime files, and are not referencing the runtime DLL
    do assert (typeof<Provider.Helpers.SomeRuntimeHelper>.Assembly.GetName().Name = asm.GetName().Name)  
    ...
    let createTypes () =
        ...
        let meth = ProvidedMethod("StaticMethod", [], typeof<Provider.Helpers.SomeRuntimeHelper>, isStatic=true, invokeCode = (fun args -> Expr.Value(null, typeof<Provider.Helpers.SomeRuntimeHelper>)))
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("BasicGenerativeProvider.DesignTime", "Provider")])
